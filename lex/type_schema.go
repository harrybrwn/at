...
type TypeSchema struct {
	...
	DefName      string `json:"-"` // parent Schema.Defs[defName] points to this *TypeSchema
	defMap       map[string]*ExtDef
	NeedsCbor    bool `json:"-"`
	NeedsType    bool `json:"-"`
	InsideAUnion bool `json:"-"`

	SchemaID string                 `json:"-"`
	FullID   string                 `json:"-"`
	Defs     map[string]*TypeSchema `json:"-"`
	Name     string                 `json:"-"`
	Parent   *TypeSchema            `json:"-"`

	Type        Type         `json:"type"`
	Key         string       `json:"key,omitempty"`
	Description string       `json:"description,omitempty"`
	Parameters  *TypeSchema  `json:"parameters,omitempty"`
	Input       *InputType   `json:"input,omitempty"`
	Output      *OutputType  `json:"output,omitempty"`
	Record      *TypeSchema  `json:"record,omitempty"`
	...
	// Format shows up when its a property.
...
func (s *TypeSchema) IsRequired(param string) bool {
	...
	if s.Type == "query" {
		...
		if s.Parameters != nil {
			...
			orderedMapIter[*TypeSchema](s.Parameters.Properties, func(k string, t *TypeSchema) error {
				...
				case "array":
					paramtypes = append(paramtypes, k+"[]"+string(t.Items.Type))
				default:

...
func TestQueue(t *testing.T) {
	...
	if q.Len() != 3 {
		t.Errorf("expected len of 3, got %d", q.Len())
	}
	...
	if q.Len() != 2 {
		t.Errorf("expected len of 2, got %d", q.Len())
	}
	...
	if q.Len() != 1 {
		t.Errorf("expected len of 1, got %d", q.Len())
	}
	...
	if q.Len() != 0 {
		t.Errorf("expected len of 0, got %d", q.Len())
	}
	if _, ok := q.Pop(); ok {
		t.Error("expected empty")
	}
